...
The following code examples feature content from the [Simple React Blog](https://www.cosmicjs.com/apps/simple-react-blog) app template. You can run the code examples as is or install the template to your account then switch `YOUR_BUCKET_SLUG` and `YOUR_BUCKET_READ_KEY` to your Bucket access keys. You can find these values located in <i>Your Bucket > Settings > API Access</i> after logging in to your <a href="https://app.cosmicjs.com">Cosmic dashboard</a>.
