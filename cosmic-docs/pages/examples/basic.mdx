...
We'll be using the [Simple React Blog](https://www.cosmicjs.com/apps/simple-react-blog) app template for the following examples. You can:

1. Run the code examples as is.
2. Install the template demo content into a Bucket on your account then switch `YOUR_BUCKET_SLUG` and `YOUR_BUCKET_READ_KEY` to your Bucket access keys. You can find these values located in <i>Your Bucket > Settings > API Access</i> after logging in to your <a href="https://app.cosmicjs.com">Cosmic dashboard</a>.

<Callout emoji="💡">
  **Quick Tip**
  <br />
  Use `props` to limit the payload size and get only the data you need. Omit
  this parameter to get the full Object response.
</Callout>
