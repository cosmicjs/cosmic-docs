...
<div label="REST">
```bash
GET $BASE_URL/buckets/${YOUR_BUCKET_SLUG}/objects?read_key=${YOUR_BUCKET_READ_KEY}&query=${query}
```
...
```bash
endpoint = "$BASE_URL/buckets/${YOUR_BUCKET_SLUG}/objects?read_key=${YOUR_BUCKET_READ_KEY}&props=title,slug,metadata"
```
...
```bash
curl '$endpoint&query={"id":"valid_object_id"}'
```
