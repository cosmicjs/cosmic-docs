...
<div label="REST">
```bash
GET https://api.cosmicjs.com/v2/buckets/${YOUR_BUCKET_SLUG}/objects?read_key=${YOUR_BUCKET_READ_KEY}&query=${query}
```
...
```bash
endpoint = "https://api.cosmicjs.com/v2/buckets/${YOUR_BUCKET_SLUG}/objects?read_key=${YOUR_BUCKET_READ_KEY}&props=title,slug,metadata"
```
