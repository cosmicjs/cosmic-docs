...
| query           |          |  JSON  |           | A JSON string to perform advanced queries to filter out objects. Please see [Examples](/cosmic-api/advanced-query#example-queries) for more detail.                                                          |
...
```bash
GET $BASE_URL/buckets/${bucket_slug}/objects?limit=1
```
...
```bash
GET $BASE_URL/buckets/${bucket_slug}/objects?limit=1&props=${props}
```
