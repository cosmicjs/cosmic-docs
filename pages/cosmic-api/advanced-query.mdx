...
Advanced queries give you powerful NoSQL database-like functionality for content fetching. Use the query parameter to send a valid JSON (stringified) query on the [Get Objects Endpoint](/cosmic-api/objects#get-objects). See [example requests below](/cosmic-api/advanced-query/#example-queries).

<Callout emoji="✅">
  <strong>Note</strong>
  If accessing the endpoint via browser address bar, open and closed curly braces
  will need to be encoded: `%7B` and `%7D` respectively. [This encoder tool may help](https://meyerweb.com/eric/tools/dencoder/).
</Callout>

**Definition**

```bash
GET $BASE_URL/buckets/${bucket_slug}/objects?read_key=${read_key}&query=${JSON_query}
```
