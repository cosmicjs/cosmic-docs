...
Queries give you powerful NoSQL database-like functionality for content fetching. Use the query parameter to send a valid JSON (stringified) query on the [Get Objects Endpoint](/api-reference/objects#get-objects). See [example requests below](/api-reference/queries/#example-queries).
